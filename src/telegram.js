...
export class TelegramBot {
  ...
  async registerCommands() {
    ...
    const commands = [
      ...
      { command: 'relic', description: '💍 Auto craft+equip relic' },
      ...
      { command: 'pause', description: '⏸ Stop autopilot (play manually)' },
      { command: 'stop', description: '⏸ Stop autopilot (alias of /pause)' },
  ...
  formatStatus(snapshot, market) {
    ...
    const lines = [
      ...
      `💎 Gems: <b>${num(snapshot.gems)}</b>${Number(snapshot.bound_gems) > 0 ? ` <i>(${num(Number(snapshot.gems) - Number(snapshot.bound_gems))} sellable · ${num(snapshot.bound_gems)} 🔒bound)</i>` : ' <i>(all sellable)</i>'}`,
      `🔹 Shards: ${num(snapshot.shards)} <i>(currency, ≠ mana_shard)</i>`,
